...
extern atomic64_t each_exit_counter[69];
...
static int __vmx_handle_exit(struct kvm_vcpu *vcpu, fastpath_t exit_fastpath)
{
	...
	extern atomic_long_t total_cpu_cycles[69];
	struct vcpu_vmx *vmx = to_vmx(vcpu);
	...
	if(exit_reason.basic <69){
	 atomic64_inc(&each_exit_counter[exit_reason.basic]);
	 atomic64_add((rdtsc()-start_time), &total_cpu_cycles[exit_reason.basic]);
	 }
